...
namespace Merge
{
    public partial class SummaryByInsurerOverall : Form
    {
        List<t_dmms_master> selectBranch = new List<t_dmms_master>();
        HSSFWorkbook hssfworkbook;
        public SummaryByInsurerOverall()
        {
            InitializeComponent();
        }

        private void SummaryByInsurerOverall_Load(object sender, EventArgs e)
        {
            ...
            var selectBranch1 = (from c in mdl.t_dmms_master
                                 ...
                                 group c by new {  c.INSURANCE_COMPANY } into g
                                 select new {  g.Key.INSURANCE_COMPANY, NumberofAccountActive = g.Count(), Total = g.Count() });
            var selectBranch2 = (from c in mdl.t_dmms_master
                                 where (c.CERT_END_DATE.CompareTo(nowstr) < 0)
                                 group c by new {  c.INSURANCE_COMPANY } into g
                                 select new {  g.Key.INSURANCE_COMPANY, NumberofAccountExpire = g.Count(), Total = g.Count() });
            var unions = from post in selectBranch2
                         join meta in selectBranch1 on new {  post.INSURANCE_COMPANY } equals new {  meta.INSURANCE_COMPANY } into gj
                         from row in gj.DefaultIfEmpty()
                         select new { post.INSURANCE_COMPANY, post.NumberofAccountExpire, NumbersofAccountActive = (row.NumberofAccountActive == null) ? 0 : row.NumberofAccountActive };
        ...
        private void saveFileDialog1_FileOk(object sender, CancelEventArgs e)
        {
            try
            {
                ...
                var unions = from post in selectBranch2
                             ...
                             select new { post.INSURANCE_COMPANY, post.NumberofAccountExpire, NumbersofAccountActive = (a.NumberofAccountActive == null) ? 0 : a.NumberofAccountActive };
                int i = 1;
                foreach (var dmms in unions)
                {
                    ...
                    row.CreateCell(1).SetCellValue(dmms.NumberofAccountExpire);
                    row.CreateCell(2).SetCellValue(dmms.NumbersofAccountActive);
                    i++;
                }
